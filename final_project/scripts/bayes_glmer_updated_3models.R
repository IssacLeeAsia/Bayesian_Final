...
# random intercept models with predictors
...
# random intercept models without predictors
...
# random slope models
model_developed_3 <- stan_glmer(
  life_expectancy ~ log_gdp + (log_gdp | country),
  data = life_expectancy_data_developed,
  family = gaussian,
  prior_intercept = normal(72, 8),
...
mcmc_trace(model_developed_3) +
  ggtitle('mcmc trace plot for developed country')
mcmc_dens_overlay(model_developed_3) +
  ggtitle('mcmc density plot for developed country')
pp_check(model_developed_3)

tidy(model_developed_3, effects = c("fixed", "ran_vals"),
     conf.int = TRUE, conf.level = 0.95)
rhat(model_developed_3)
...
mcmc_trace(model_developing_3, size = 0.1) +
  ggtitle('mcmc trace plot for developing country')
mcmc_dens_overlay(model_developing_3) +
  ggtitle('mcmc density plot for developing country')
pp_check(model_developing_3)

tidy(model_developing_3, effects = c("fixed", "ran_vals"),
     conf.int = TRUE, conf.level = 0.95)
rhat(model_developing_3)
...
pred_5 <- posterior_predict(model_developed_3)
pred_6 <- posterior_predict(model_developing_3)

# developed, with predictors
p1 <- ppc_ribbon(life_expectancy_data_developed$life_expectancy, pred_1)
p2 <- ppc_intervals(life_expectancy_data_developed$life_expectancy, pred_1)
grid.arrange(p1, p2, nrow = 2)

# developing, with predictors
p3 <- ppc_ribbon(life_expectancy_data_developing$life_expectancy, pred_2)
p4 <- ppc_intervals(life_expectancy_data_developing$life_expectancy, pred_2)
grid.arrange(p3, p4, nrow = 2)

# developed, without predictors
p5 <- ppc_ribbon(life_expectancy_data_developed$life_expectancy, pred_3)
p6 <- ppc_intervals(life_expectancy_data_developed$life_expectancy, pred_3)
grid.arrange(p5, p6, nrow = 2)

# developing, without predictors
p7 <- ppc_ribbon(life_expectancy_data_developing$life_expectancy, pred_4)
p8 <- ppc_intervals(life_expectancy_data_developing$life_expectancy, pred_4)
grid.arrange(p7, p8, nrow = 2)

# developed, random slope
p9 <- ppc_ribbon(life_expectancy_data_developed$life_expectancy, pred_5)
p10 <- ppc_intervals(life_expectancy_data_developed$life_expectancy, pred_5)
grid.arrange(p9, p10, nrow = 2)

# developing, random slope
p11 <- ppc_ribbon(life_expectancy_data_developing$life_expectancy, pred_6)
p12 <- ppc_intervals(life_expectancy_data_developing$life_expectancy, pred_6)
...
developed_3_loo <- loo(model_developed_3)
loo_compare(developed_1_loo, developed_2_loo, developed_3_loo)

# developing 
developing_1_loo <- loo(model_developing_1)
developing_2_loo <- loo(model_developing_2)
developing_3_loo <- loo(model_developing_3)
loo_compare(developing_1_loo, developing_2_loo, developing_3_loo)
